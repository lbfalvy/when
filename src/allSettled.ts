...
range(5).map(n => `export default function allSettled<${range(n+1).map(k => `T${k}`).join(', ')}>(input: [${range(n+1).map(k => `Thenable<T${k}>`).join(', ')}]): XPromise<[${range(n+1).map(k => `SettledResult<T${k}>`).join(', ')}]>;`).join("\n")

Note: it's only generated up to 5 parameters because otherwise Typescript exceeds its recursion limit.
*/

export default function allSettled<T0>(input: [Thenable<T0>]): XPromise<[SettledResult<T0>]>;
export default function allSettled<T0, T1>(input: [Thenable<T0>, Thenable<T1>]): XPromise<[SettledResult<T0>, SettledResult<T1>]>;
